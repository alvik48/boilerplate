...
link_for() {
  ...
  if [ -d "$link" ] && [ ! -L "$link" ]; then
    echo "skills:link — $link is a real directory; leaving it alone." >&2
  ...
  if ln -s ../.agents/skills "$link" 2>/dev/null; then
    echo "skills:link — $link -> ../.agents/skills"
  else
    # Windows needs Developer Mode or elevation to create symlinks.
    echo "skills:link — could not symlink $link (on Windows, enable Developer Mode" >&2

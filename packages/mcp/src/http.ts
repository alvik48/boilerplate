...
export function createMcpHandler(options: ServerOptions, origin: string, budget = 120) {
  let window = Date.now();
  let count = 0;
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (signal.aborted) return new Response('Request timed out or cancelled', { status: 408 });
      throw error;
    } finally {

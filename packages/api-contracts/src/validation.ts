...
export const operations = (spec: OpenAPIV3.Document) => {
...
export const validateOpenApi = async (input: unknown): Promise<OpenAPIV3.Document> => {
  ...
  if (!('openapi' in spec) || !spec.openapi.startsWith('3.0.')) {
    throw new Error('Expected OpenAPI 3.0.x');
  }
  ...
  const validateMedia = (content: Record<string, OpenAPIV3.MediaTypeObject>) => {
    for (const media of Object.values(content)) {
      if (!media.schema) {
        throw new Error('Missing media schema');
      }

      if (media.example !== undefined) {
        validateExample(document, media.schema, media.example);
      }

      for (const example of Object.values(media.examples ?? {})) {
        if (!('$ref' in example) && example.value !== undefined) {
          validateExample(document, media.schema, example.value);
        }
      }
    }
  };
  ...
  for (const { path, method, operation } of operations(document)) {
    ...
    if (operation.requestBody && !('$ref' in operation.requestBody)) {
      validateMedia(operation.requestBody.content);
    }
    ...
    for (const parameter of [...(document.paths[path]?.parameters ?? []), ...(operation.parameters ?? [])]) {
      ...
      if (parameter.content) {
        validateMedia(parameter.content);
      }
    }
    ...
    for (const response of Object.values(operation.responses)) {
      ...
      if (!response.description) {
        throw new Error(`Missing response description: ${operation.operationId}`);
      }
  ...
  if (!ids.size) {
    throw new Error('HTTP service has no documented operations');
  }
...
export const validateExample = (
  ...
  if (!validate(example)) {
    throw new Error(`Invalid contract example: ${ajv.errorsText(validate.errors)}`);
  }
};

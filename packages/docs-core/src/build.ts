...
import { type Document, type Manifest, type Metadata, metadataSchema, type Section } from './model.js';
...
export const discover = async (directory: string): Promise<string[]> => {
  const entries = await readdir(directory, { withFileTypes: true });
  const files = await Promise.all(
    entries.map(async (entry) => {
      if (entry.isSymbolicLink()) {
        throw new Error(`Symlink in public documentation: ${entry.name}`);
      }
...
export const documentPath = (source: string) => {
  return (
    '/docs' +
    source
      .replace(/^docs/, '')
...
export const buildManifest = async (options: BuildOptions): Promise<Manifest> => {
  const { root } = options;
  const origin = new URL(options.origin).origin;
  const sourceUrl = new URL(options.sourceUrl);

  if (sourceUrl.protocol !== 'https:') {
    throw new Error('Repository source URL must use HTTPS');
  }
  ...
  for (const file of await discover(path.join(root, 'docs'))) {
    ...
    if (!['integration', 'api', 'repository'].includes(doc.section)) {
  ...
  for (const field of ['id', 'path', 'sourcePath'] as const) {
    ...
    for (const doc of docs) {
      ...
      if (seen.has(key)) {
        throw new Error(`Duplicate document ${field}: ${doc[field]}`);
      }
  ...
  for (const association of associations) {
    if (contractIds.has(association.id)) {
      throw new Error(`Duplicate contract ID: ${association.id}`);
    }

    contractIds.add(association.id);

    if (!association.guideIds.length) {
      throw new Error(`No integration coverage: ${association.id}`);
    }
    ...
    for (const id of association.guideIds) {
      ...
      if (association.referenceId) {
        guide.related.push(association.referenceId);
      }
    }
  }

  for (const contract of options.contracts ?? []) {
    if (!contract.owner || !contract.version || !contract.id) {
      throw new Error('Incomplete external contract');
    }
  ...
  for (const doc of docs) {
    for (const example of doc.examples) {
      ...
      if (!service || !schema || !block) {
        throw new Error(`Invalid example mapping in ${doc.id}`);
      }
    ...
    for (const id of doc.related) {
      if (!byId.has(id)) {
        throw new Error(`Unknown related document ${id} in ${doc.id}`);
      }
    }
      ...
      if (target) {
        ...
        if (rawPath.startsWith('/')) {
          if (
            ...
            throw new Error(`Unknown public route: ${url} in ${doc.id}`);
          }

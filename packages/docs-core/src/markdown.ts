...
export function inspectMarkdown(markdown: string): { tree: Root; headings: Document['headings'] } {
...
export function normalizeMarkdown(markdown: string, resolve: (url: string) => string) {
  const { tree } = inspectMarkdown(markdown);
  visit(tree, (node) => {
    if (node.type === 'link' || node.type === 'image' || node.type === 'definition') node.url = resolve(node.url);
    if (node.type === 'html') throw new Error('Use ordinary Markdown instead of raw HTML in public docs');
  });

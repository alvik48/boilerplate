...
export async function validateInventory(root: string, services: ServiceRegistration[]) {
  const turbo = z
  ...
  for (const area of ['apps', 'templates', 'packages']) {
    for (const entry of await readdir(path.join(root, area), { withFileTypes: true })) {
      ...
      if (pkg.apiContract?.kind !== 'http') continue;
  ...
  if (found.size !== services.length) throw new Error('Registry contains a missing workspace HTTP service');

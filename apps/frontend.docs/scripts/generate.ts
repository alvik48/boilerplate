...
import { services, externalContracts } from '../services.js';
...
export async function generate() {
  ...
  for (const service of services) {
    ...
    if (spec.info.version !== service.version) throw new Error(`Service version mismatch: ${service.id}`);
    ...
    for (const id of Object.keys(service.guides))
      if (!ids.has(id)) throw new Error(`Stale operation mapping: ${service.id}/${id}`);
...
export async function publish() {
  ...
  for (const file of await discover(assetRoot)) if (!assets.has(file)) await rm(file);
  for (const api of manifest.apis) files.set(`openapi/${api.id}.json`, stableJson(api.spec));
  files.set('manifest.json', stableJson(manifest));
  await mkdir(output, { recursive: true });
  for (const [file, content] of files) await writeChanged(path.join(output, file), content);
  for (const file of await discover(output)) if (!files.has(path.relative(output, file))) await rm(file);
